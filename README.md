...
![Python](https://img.shields.io/badge/pip_install-mdm--parser-blue)
...
# OCR (scanned images → text)
ocr = OcrProcessor(engine="auto", lang="kor+eng")
text = ocr.extract_text("scanned.png")
```
...
> **Note**: `mdm-core` (Rust-powered) is coming soon. For now, use `pip install mdm-parser` for Python-native parsing.

```bash
pip install mdm-core[langchain]  # Coming soon
```
...
```bash
pip install mdm-core[llamaindex]  # Coming soon
```
...
│       └── main.rs          #   CLI tool
├── packages/
│   ├── python/              # [Python] mdm-core (Rust bindings, coming soon)
│   │   └── python/mdm_core/ #   LangChain, LlamaIndex loaders
│   └── parser-py/           # [Python] pip install mdm-parser (available now)
├── viewer/
...
### v0.1 — Foundation (현재)
- 9개 포맷 변환 (HWP/HWPX/PDF/DOCX/PPTX/XLSX/HTML/CSV/TXT)
- Rust core + Python (`pip install mdm-parser`) + CLI
- MCP gateway (Korea Law Hub)
- Desktop app (Tauri, macOS)
- WASM build (`npm install @markdown-media/wasm`)

### v0.5 — Rich Extraction
- **표 병합 셀 보존** — HWP rowspan/colspan → `.mdm` sidecar에 구조 보존
- **중첩 표** — 표 안의 표를 참조 기반으로 추출
- **수식** — HWP EQEDIT → LaTeX/MathML 변환
- `hwp2mdm inspect` — 변환 품질 디버깅 CLI
- GitHub Pages 데모 (WASM, 설치 없이 체험)

### v1.0 — Complete MDM Format
- `.mdx` + `.mdm` 사이드카 포맷 정식 스펙
- `@markdown-media/viewer` — MDM 뷰어 (브라우저 렌더러)
- 병합 표/수식/중첩 구조의 완전한 HTML 렌더링
- VS Code 확장 (HWP → Markdown 변환)
- Pandoc 품질 벤치마크 자동화

### v2.0 — Ecosystem
- 플러그인 아키텍처 (커스텀 포맷 추가)
- 실시간 협업 뷰어
- LangChain/LlamaIndex 공식 통합
- 다국어 OCR 강화

> **Scope**: MDM은 **기존 문서 → Markdown 단방향 파이프라인**입니다. Markdown을 HWP/DOCX로 역변환하는 기능은 범위 밖입니다.
...
- **Python統合** — `pip install mdm-parser` でパーサーを即座に利用可能
...
- **Python集成** — `pip install mdm-parser` 即可使用解析器
