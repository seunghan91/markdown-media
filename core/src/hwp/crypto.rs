...
mod tests {
    ...
    #[test]
    fn aes128_ecb_nist_kat() {
        // NIST FIPS-197 Appendix B known-answer test (ported from
        // reference/kordoc/tests/crypto.test.ts).
        ...
        let out = aes128_ecb_decrypt(&ciphertext, &key);
        assert_eq!(out, expected);
        // Two identical blocks decrypt identically under ECB.
        let two = [ciphertext, ciphertext].concat();
        let out2 = aes128_ecb_decrypt(&two, &key);
        assert_eq!(&out2[..16], &expected[..]);
        assert_eq!(&out2[16..], &expected[..]);
    }
